...
//    YoutubeExampleApplication.get(this).getYouTube()
    ...
    rxTube.create(new RxTube.Query<YouTube.Search.List>() {
      @Override public YouTube.Search.List create(YouTube youTube) throws Exception {
        return youTube.search().list("skateboarding");
      }
    }).observeOn(AndroidSchedulers.mainThread()).subscribe(new Action1<SearchListResponse>() {
      @Override public void call(SearchListResponse searchListResponse) {
